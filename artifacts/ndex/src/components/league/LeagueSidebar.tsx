...
export default function LeagueSidebar({ league, section, onSelect }: Props) {
  const [expanded, setExpanded] = useState<Set<string>>(new Set(["players"]));
  ...
  return (
    <aside className="w-[200px] shrink-0 bg-[#0d0d0d] border-r border-white/8 flex flex-col overflow-y-auto">
      {/* League identity */}
                  ...
                  {item.sub.map((sub) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-4 py-1.5 text-[11px] transition-colors ${
                          section === sub.key
                            ? "text-[#00C8FF]"
